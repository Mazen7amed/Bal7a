...
if option == 'Home':
    ...
    df['Clicked'] = df['Clicked'].astype('O')  
    fig = px.bar(data_frame=df, x='Clicked')
    st.plotly_chart(fig)
    fig = plt.figure()
    df['Clicked'].value_counts().plot(kind='bar')
    st.pyplot(fig)
    fig = px.histogram(data_frame=df, x='Time Spent on Site', nbins=10, title='Time Spent on Site Distribution')
    st.plotly_chart(fig)
    
    fig = px.pie(data_frame=df, names='Clicked', title='Distribution of Clicked')
    st.plotly_chart(fig)
elif option == "ML":
